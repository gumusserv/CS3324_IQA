...
def build_model(input_shape_image, input_shape_augmentation_image, input_shape_name):
    ...
    merged = concatenate([input_image, input_augmentation_image, input_name])

    x1 = Dense(128, activation='sigmoid')(merged)
    x2 = Dropout(0.2)(x1)
    ...
    model = Model(inputs=[input_image, input_augmentation_image, input_name], outputs=[quality_mapped_output])
...
if __name__ == '__main__':
    ...
    # quality_scores = filtered_data1['mos_quality']   # 替换为实际的列名
    quality_scores = data1['mos_quality']
    ...
    # quality_scores2 = filtered_data2['mos_quality']
    ...
    name_features1 = extract_text_features(data1['name'])
    ...
    for fold, (train_index, test_index) in enumerate(kf.split(image_features)):
        ...
        # 训练模型
        model.fit([X_image_train, X_augment_train, X_name_train], [y_quality_train], epochs=30, batch_size=32)

        # 性能评估
        quality_pred = model.predict([X_image_test, X_augment_test, X_name_test])
    ...
    # model.save("model_total", save_format='tf')
